...
export const createNewJobModel = function (context: BaseContext): Job {
    ...
    job.address.streetName = context.request.body.address.streetName || ''
    job.address.streetNumber = context.request.body.address.streetNumber || ''
    job.address.streetBus = context.request.body.address.streetBus || ''
    job.address.city = context.request.body.address.city || ''
    job.address.postalCode = context.request.body.address.postalCode || ''
    job.address.country = context.request.body.address.country || ''
...
export const createUpdateJobModel = function (context: BaseContext): Job {
    ...
    job.experience = context.request.body.experience
    job.responsible = context.request.body.experience
    job.address = context.request.body.address
    job.description = context.request.body.description
    job.applicantForm = context.request.body.applicantForm
    job.applicantProcess = context.request.body.applicantProcess
    job.organizationId = context.request.body.organizationId
    job.status = context.request.body.status || '1'
